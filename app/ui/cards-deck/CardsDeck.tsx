...
/**
 * Direction of a swipe based on dx sign.
 */
...
export interface SwipeProgressInfo {
    ...
    dx: number;
    /** Progress from 0..1 based on the commit threshold distance (NOT full width). */
    progress: number;
...
export interface SwipeActionVisual {
    ...
    /** Icon rendered inside overlay (vector or svg). */
    icon: IconProps;

    /**
     * Max overlay width ratio relative to deck width.
     * Clamped by overlayMaxWidthRatio.
     *
     * @defaultValue 0.3
     */
    widthRatio?: number;
 ...
 *
 * UI-only controlled deck:
 * parent provides front/back items (2 items only → memory friendly).
 */
export interface CardsDeckProps<TFront, TBack = TFront> {
    ...
    /** Renders front card content. */
    renderFront: (item: TFront) => React.ReactNode;

    /** Renders back card content. Optional. If missing, a placeholder is rendered. */
    ...
    /** Called once when swipe auto-commits. */
    onSwipeCommit?: (direction: SwipeDirection) => void;

    /** Called if swipe is cancelled (release before threshold). */
    onSwipeCancel?: () => void;

    /** LEFT revealed gap overlay (swipe RIGHT → approve). */
    leftAction?: SwipeActionVisual;

    /** RIGHT revealed gap overlay (swipe LEFT → trash). */
    rightAction?: SwipeActionVisual;

    /**
     * Adds subtle depth (scale) on the back card.
     * @defaultValue true
     */
...
/**
 * CardsDeck: 2-card stack with auto-commit at threshold.
 *
 * - Back card exists to host the revealed overlays.
 * - Front card is draggable.
 * - Only 2 cards mounted → optimized for 1000+ media items.
 */
export default function CardsDeck<TFront, TBack = TFront>({
    ...
    const backContent =
        backItem !== undefined
            ? (renderBack ? renderBack(backItem) : <View style={{ flex: 1 }} />)
            : null;
